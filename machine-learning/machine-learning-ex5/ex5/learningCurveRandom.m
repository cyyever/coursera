...
for i = 1:m
   ...
   for j = 1:random_cnt 
       rows = randi(m,[1,i]);
       theta = trainLinearReg(X(rows,:), y(rows), lambda);
       error_train(i) = error_train(i)+linearRegCostFunction(X(rows,:), y(rows), theta, 0);   
       error_val(i) = error_val(i)+linearRegCostFunction(Xval, yval, theta, 0);   
   end
   error_train(i) = error_train(i)/random_cnt;
   error_val(i) = error_val(i)/random_cnt;
end
